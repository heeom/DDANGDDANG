...
//			JdbcUtil.close(pstmt);
	...
	public int updateReArticle(qnaBean article_re) {
		...
		try {
			String sql = "UPDATE support_board set wr_content=?,wr_datetime=now() where wr_id=? and wr_co_id=1; ";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, article_re.getWr_content());
			pstmt.setInt(2, article_re.getWr_id());
