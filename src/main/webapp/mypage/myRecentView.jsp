...
<c:when test="${!(pageInfo.listCount eq 0)}">
	...
	<c:otherwise>
		<div class="empty"> 최근 본 상품 내역이 없습니다. 실시간 경매 물품을 구경해보세요:)<br>
		<span class="gotoLink">
		<a href="<c:url value='../item/list.it'/>">[상품 구경하러 가기]</a>
		</span>
		</div> 
	</c:otherwise>
</c:choose>
...
<script type="text/javascript">
	$(document).ready(function(){	
		...
		$('.page').click(function(){
			var page = $(this).val();
			$('.show_page').load("MyRecentView.mk?page="+page);
		});
		
		$('.Prev').click(function(){
			var prevPage = parseInt(nowPage)-1;
			$('.show_page').load("MyRecentView.mk?page="+prevPage);
		});
		
		$('.Next').click(function(){
			var nextPage = parseInt(nowPage)+1;
			$('.show_page').load("MyRecentView.mk?page="+nextPage);
		});
});
</script>
</html>