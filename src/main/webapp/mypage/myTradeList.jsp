...
<c:when test="${!(pageInfo.listCount eq 0)}">		
		...
		<c:forEach var="itemList" items="${item }" begin="0" end="${item.size() }">
			...
			${itemList.name }</a><br>
			<fmt:formatNumber value="${itemList.successfulPrice}" pattern="#,###원" /><br>
	...
	<c:otherwise>
		<div class="empty"> 낙찰 내역이 없습니다. 입찰을 시작해보세요 :)<br>
		<span class="gotoLink">
		<a href="<c:url value='../item/list.it'/>">[입찰 하러 가기]</a>
		</span>
		</div> 
	</c:otherwise>
</c:choose>
...
<script type="text/javascript">
	$(document).ready(function(){	
		...
		$('.page').click(function(){
			var page = $(this).val();
			$('.show_page').load("MyTradeList.mk?page="+page);
		});
		
		$('.Prev').click(function(){
			var prevPage = parseInt(nowPage)-1;
			$('.show_page').load("MyTradeList.mk?page="+prevPage);
		});
		
		$('.Next').click(function(){
			var nextPage = parseInt(nowPage)+1;
			$('.show_page').load("MyTradeList.mk?page="+nextPage);
		});
});
</script>
</html>